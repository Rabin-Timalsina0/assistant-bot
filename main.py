...
model = None
qdrant = None
INSTAAGRAM_PAGE_ID = "17841478811453634"
@asynccontextmanager
async def lifespan(app: FastAPI):
    global app_ready, model, qdrant
    ...
    try:
        # Initialize Qdrant client (async to prevent blocking)
        print("Initializing Qdrant client...")
        qdrant_url = os.getenv("QDRANT_URL")
        qdrant_api_key = os.getenv("QDRANT_API_KEY")
        
        if not qdrant_url or not qdrant_api_key:
            print("Warning: QDRANT_URL or QDRANT_API_KEY not set. Some features may not work.")
            qdrant = None
        else:
            try:
                qdrant = QdrantClient(
                    ...
                    timeout=10,  # Very short timeout for startup
                    prefer_grpc=False
                )
                ...
                print(f"Qdrant connection failed during startup: {e}")
                qdrant = None
        
        # Load the model with error handling
        print("Loading SentenceTransformer model...")
        try:
            # Use cached model path for faster loading
            model_path = "/app/models/clip-ViT-B-32"
        ...
        # Pre-warm database connections if needed
        if hasattr(database, 'prewarm_connections'):
            database.prewarm_connections()
        
        # Mark app as ready even if some components failed
        app_ready = True
        print("Application initialization complete - ready to serve traffic")
        
    except Exception as e:
        print(f"Startup error: {e}")
        # Still mark as ready to prevent infinite startup loop
        app_ready = True
        print("Application marked as ready despite startup issues")
...
app = FastAPI(
    ...
    description="API for business chatbot functionality",
    version="1.0.0"
)
app.add_middleware(
    CORSMiddleware,
...
async def root_check():
    ...
    return {
        "status": "running", 
        "message": "Business Chatbot API is running",
        "timestamp": time.time()
    }

@app.get("/health")
async def health_check():
    """Health check endpoint for Cloud Run - always returns 200"""
    global app_ready, model, qdrant
    ...
    # Only check Qdrant if it's available
    if qdrant is not None:
        try:
            qdrant.get_collections()
            health_status["qdrant_responsive"] = True
        except Exception as e:
            health_status["qdrant_responsive"] = False
...
async def readiness_check():
    """Readiness check - returns ready as soon as app starts"""
    global app_ready, model, qdrant
    
    # Mark as ready immediately to prevent startup probe timeouts
    # The app can handle requests even if some components aren't fully loaded
    checks = {
        "app_ready": app_ready,
        "model_loaded": model is not None,
        "qdrant_connected": qdrant is not None,
    ...
    # Always return 200 to prevent startup probe failures
    # Individual components will handle their own errors gracefully
    return {"status": "ready", "checks": checks}

@app.get("/live")
async def liveness_check():
    """Liveness check endpoint - always returns 200 if app is running"""
    return {"status": "alive"}

@app.get("/startup")
async def startup_check():
    """Startup check - responds immediately for Cloud Run startup probes"""
